...
const TabsBar = () => {
  ...
  <div className="flex overflow-x-auto whitespace-nowrap flex-1 min-w-0">
    {tabs.map((tab) => (
      <div
        key={tab.id}
      ...
      >
        <span
          className={`mr-2 text-[9px] font-medium ${
            tab.method === "GET"
              ? "text-green-400"
              : tab.method === "POST"
              ? "text-yellow-400"
        ...
        <TabTitleEdit
          ...
          onSave={(id, newTitle) => updateTabTitle(id, newTitle)}
          trigger={<span className="max-w-xs text-[12px] truncate">{tab.title}</span>}
        />
        <Tooltip>
  ...
  <Tooltip>
    <TooltipTrigger asChild>
      <button
        className="px-1 py-1 ml-2 mr-10 rounded hover:bg-[#282828] text-gray-300 cursor-pointer flex-shrink-0"
        onClick={addTab}
      >
        <Plus size={14} />
      </button>
